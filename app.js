...
function renderSetup(invalid=false,error=''){
  ...
  bindTop();
  $('#saveFirebaseBtn').onclick=()=>{try{let raw=$('#firebaseConfig').value.trim();const start=raw.indexOf('{'),end=raw.lastIndexOf('}');if(start>=0&&end>start)raw=raw.slice(start,end+1);raw=raw.replace(/([a-zA-Z0-9_]+)\s*:/g,'"$1":').replace(/'/g,'"');const cfg=JSON.parse(raw);if(!cfg.apiKey||!cfg.projectId)throw new Error('apiKey/projectId manquants');storeFirebaseConfig(cfg);location.reload()}catch(e){alert('Impossible de lire la configuration. Colle uniquement l’objet firebaseConfig.\n\n'+e.message)}};
